...
CREATE TABLE IF NOT EXISTS `mydb`.`Horario` (
  ...
  PRIMARY KEY (`id`),
  INDEX `semana_idx` (`dias_da_semana` ASC) VISIBLE,
  CONSTRAINT `semana_horario`
    FOREIGN KEY (`dias_da_semana`)
    REFERENCES `mydb`.`Dias_da_semana` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Funcionario` (
  ...
  `data_de_nascimendo` DATE NOT NULL,
  `genero` CHAR(1) NOT NULL,
  `funcao` INT NULL,
  PRIMARY KEY (`numero`),
  INDEX `funcao_idx` (`funcao` ASC) VISIBLE,
  CONSTRAINT `funcao_funcionario`
    FOREIGN KEY (`funcao`)
...
CREATE TABLE IF NOT EXISTS `mydb`.`Aviao` (
  ...
  `id_lugar_local` INT NULL,
  `icao_atual` CHAR(4) NULL,
  PRIMARY KEY (`marcas_da_aeronave`),
  UNIQUE INDEX `id_lugar_local_UNIQUE` (`id_lugar_local` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Manutencao` (
  `id_funcionario` INT NOT NULL,
  `id_aviao` VARCHAR(20) NOT NULL,
  `data_inicio` DATETIME NOT NULL,
  `duracao` TIME NOT NULL,
  `tarefa` INT NOT NULL,
  `cancelado` TINYINT NOT NULL,
  `observacoes` TEXT NULL,
  PRIMARY KEY (`id_funcionario`, `id_aviao`),
  INDEX `tipo_idx` (`tarefa` ASC) VISIBLE,
  INDEX `id_aviao_idx` (`id_aviao` ASC) VISIBLE,
  CONSTRAINT `tipo_manutencao_aviao`
    FOREIGN KEY (`tarefa`)
    REFERENCES `mydb`.`Tarefa` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_funcionario_manutencao_aviao`
    FOREIGN KEY (`id_funcionario`)
    REFERENCES `mydb`.`Funcionario` (`numero`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_aviao_manutencao_aviao`
...
-- -----------------------------------------------------
-- Table `mydb`.`Servico_aviao`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`Servico_aviao` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Servico_aviao` (
...
CREATE TABLE IF NOT EXISTS `mydb`.`Cliente` (
  ...
  INDEX `data_quotas_idx` (`quotas` ASC) VISIBLE,
  CONSTRAINT `data_quotas`
    FOREIGN KEY (`quotas`)
    REFERENCES `mydb`.`Quotas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
